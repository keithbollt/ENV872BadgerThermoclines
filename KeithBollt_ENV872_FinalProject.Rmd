...
fontsize: 12pt
---
Steps to turning this thing in:
-Turn off warning messages at the top of the document
-Run the models but don't include the results in the code
-Make sure code doesnt run off page- go to the text on the pdf and press enter in the rmd
-Create tables that summarize the results of the models (probably 3: 2 for smk tests, temp and oxygen)
-Move the temperature of all lakes plot to the ANOVA result section
-Make an identical plot for oxygen
-Number all the tables and plots
-Add the template elements from the project template rmd
-knit 
-knit readme file, too
-turn in both files
...
See below the temperature data distribution for each of the five lakes at 7m depth. This figure tells us which lakes are coldest in the summer. 
